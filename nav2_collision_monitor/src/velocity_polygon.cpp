  ...
  std::string & /*footprint_topic*/)
{
  ...
  try {
    ...
    for (std::string velocity_polygon_name : velocity_polygons) {
      ...
      // polygon points parameter (as double array)
      nav2_util::declare_parameter_if_not_declared(
        node, polygon_name_ + "." + velocity_polygon_name + ".points", rclcpp::PARAMETER_DOUBLE_ARRAY);
      std::vector<double> poly_array =
        node->get_parameter(polygon_name_ + "." + velocity_polygon_name + ".points").as_double_array();

      if (poly_array.size() % 2 != 0) {
        RCLCPP_ERROR(
          logger_,
          "[%s]: Points array for polygon [%s] must have even number of elements (pairs of x,y). Got %zu elements.",
          polygon_name_.c_str(), velocity_polygon_name.c_str(), poly_array.size());
        return false;
      }

      for (size_t i = 0; i < poly_array.size(); i += 2) {
        Point p;
        p.x = poly_array[i];
        p.y = poly_array[i + 1];
        poly.push_back(p);
