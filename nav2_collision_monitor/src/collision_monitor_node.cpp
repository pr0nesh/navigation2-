...
CollisionMonitor::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  // Create standard ROS2 subscribers and publishers
  // Subscribe to both Twist and TwistStamped messages
  ...
  cmd_vel_in_stamped_sub_ = this->create_subscription<geometry_msgs::msg::TwistStamped>(
    cmd_vel_in_topic + "_stamped", 
    1,
    std::bind(&CollisionMonitor::cmdVelInCallbackStamped, this, std::placeholders::_1));

  // Create publisher for output cmd_vel
  ...
  if (use_realtime_priority) {
    RCLCPP_WARN(get_logger(), "Real-time priority setting requested but not available in this build");
    // Note: setSoftRealTimePriority may not be available in all nav2_util versions
...
void CollisionMonitor::cmdVelInCallbackUnstamped(geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  header.frame_id = get_parameter("base_frame_id").as_string();

  process({msg->linear.x, msg->linear.y, msg->angular.z}, header);
}
