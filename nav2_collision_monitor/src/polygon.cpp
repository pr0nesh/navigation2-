  ...
  std::string & footprint_topic)
{
  ...
  try {
    try {
      ...
      return getPolygonFromString(polygon_name_ + ".points", poly_);
  ...
  std::vector<Point> & polygon)
{
  ...
  try {
    // Declare with default empty vector
    nav2_util::declare_parameter_if_not_declared(
      node, param_name, rclcpp::ParameterValue(std::vector<double>{}));

    rclcpp::Parameter param = node->get_parameter(param_name);

    // Case 1: double[] (flat list of x,y pairs)
    if (param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE_ARRAY) {
      std::vector<double> flat_points = param.as_double_array();

      if (flat_points.size() % 2 != 0) {
        RCLCPP_ERROR(
          logger_,
          "[%s]: Polygon points must be in x,y pairs. Got odd number of values.",
          polygon_name_.c_str());
        return false;
      }
      ...
      for (size_t i = 0; i < flat_points.size(); i += 2) {
        polygon.emplace_back(Point{flat_points[i], flat_points[i + 1]});
      }
      return true;
    }

    // Case 2: string[] (each element like "x,y")
    if (param.get_type() == rclcpp::ParameterType::PARAMETER_STRING_ARRAY) {
      std::vector<std::string> str_points = param.as_string_array();
      polygon.clear();
      ...
      for (const auto & s : str_points) {
        ...
        if (token.empty()) {
          RCLCPP_ERROR(logger_, "[%s]: Invalid empty token in point string \"%s\".",
                       polygon_name_.c_str(), s.c_str());
          return false;
        }
        double x = std::stod(token);

        if (!std::getline(iss, token, ',')) {
          RCLCPP_ERROR(logger_, "[%s]: Invalid point string \"%s\". Expected \"x,y\".",
                       polygon_name_.c_str(), s.c_str());
          return false;
        }
    ...
    // Case 3: single string that looks like "[0.0, 0.0, 1.0, 0.0, 1.0, 1.0]"
    if (param.get_type() == rclcpp::ParameterType::PARAMETER_STRING) {
      ...
      while (std::getline(iss, token, ',')) {
        ...
        flat_points.push_back(std::stod(token));
      }
    ...
    RCLCPP_ERROR(
      logger_,
      "[%s]: Polygon parameter must be double[], string[], or string but got unsupported type.",
      polygon_name_.c_str());
    return false;
  }
  catch (const std::exception & ex) {
    RCLCPP_ERROR(
      logger_,
      "[%s]: Failed to parse polygon points: %s",
      polygon_name_.c_str(),
      ex.what());
    return false;
  }
